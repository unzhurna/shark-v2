<?php defined('BASEPATH') OR exit('No direct script access allowed');

class Testi_model extends CI_Model{

    var $table = 'testimony';
    var $column_order = array(null, 'testi_name','testi_title', 'testi_content', null);
    var $column_search = array('testi_name','testi_name','testi_content');
    var $order = array('testi_id' => 'asc');

    private function _get_datatables_query()
    {

        $this->db->from($this->table);

        $i = 0;

        foreach ($this->column_search as $item)
        {
            if(isset($_POST['search']['value']))
            {

                if($i===0)
                {
                    $this->db->group_start();
                    $this->db->like($item, $_POST['search']['value']);
                }
                else
                {
                    $this->db->or_like($item, $_POST['search']['value']);
                }

                if(count($this->column_search) - 1 == $i)
                    $this->db->group_end();
            }
            $i++;
        }

        if(isset($_POST['order']))
        {
            $this->db->order_by($this->column_order[$_POST['order']['0']['column']], $_POST['order']['0']['dir']);
        }
        else if(isset($this->order))
        {
            $order = $this->order;
            $this->db->order_by(key($order), $order[key($order)]);
        }
    }

    function get_datatables()
    {
        $this->_get_datatables_query();
        if($_POST['length'] != -1)
        $this->db->limit($_POST['length'], $_POST['start']);
        $query = $this->db->get();
        return $query->result();
    }

    function count_filtered()
    {
        $this->_get_datatables_query();
        $query = $this->db->get();
        return $query->num_rows();
    }

    public function count_all()
    {
        $this->db->from($this->table);
        return $this->db->count_all_results();
    }

    public function get_all_testi()
    {
        $this->db->select('*');
        $this->db->from('testimony');
        $query = $this->db->get();
        return $query->result_array();
    }

    function get_detail_testi($id)
    {
        $this->db->select('*');
        $this->db->from('testimony');
        $this->db->where('testi_id', $id);
        $query = $this->db->get();
        $return = $query->row_array();
        return $return;
    }

    public function submit_form_data($data)
    {
        if(!empty($data['testi_id']))
        {
            $data['_update_at'] = date('Y-m-d h:i:s');
            $data['_update_by'] = 1;
            $this->db->set($data);
            $this->db->where('testi_id', $data['testi_id']);
            $this->db->update('testimony');
            $this->session->set_flashdata('message', 'Success, Data has been updated!');
        }
        else
        {
            unset($data['testi_id']);
            $data['_create_at'] = date('Y-m-d h:i:s');
            $data['_create_by'] = 1;
            $this->db->set($data);
            $this->db->insert('testimony');
            $this->session->set_flashdata('message', 'Success, Data has been added!');
        }
    }

    public function delete_testi($id)
    {
        $this->db->where('testi_id', $id);
        $this->db->delete('testimony');
    }

}
